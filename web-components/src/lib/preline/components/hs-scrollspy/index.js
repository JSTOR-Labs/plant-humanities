...
export class HSScrollspy extends Component {
  constructor(root) {
  ...
  init() {
    this.root.querySelectorAll(this.selector).forEach(($scrollspyEl) => {
      const $scrollspyContentEl = this.root.querySelector($scrollspyEl.getAttribute('data-hs-scrollspy'));
      ...
      const $scrollableEl = $scrollspyEl.getAttribute('data-hs-scrollspy-scrollable-parent')
        ? this.root.querySelector($scrollspyEl.getAttribute('data-hs-scrollspy-scrollable-parent'))
        : this.root;
  ...
  _update({ ev, $scrollspyEl, sections, links, $sectionEl }) {
    ...
    const offsetScrollableParent = ev.target === this.root ? 0 : parseInt(ev.target.getBoundingClientRect().top);
  ...
  _scrollTo({ $scrollspyEl, $scrollableEl, $link }) {
    const $sectionEl = this.root.querySelector($link.getAttribute('href'));
    const globalOffset = parseInt(this.getClassProperty($scrollspyEl, '--scrollspy-offset', '0'));
    const userOffset = this.getClassProperty($sectionEl, '--scrollspy-offset') || globalOffset;
    const offsetScrollableParent = $scrollableEl === this.root ? 0 : $scrollableEl.offsetTop;
    const topOffset = $sectionEl.offsetTop - userOffset - offsetScrollableParent;
    const $viewEl = $scrollableEl === this.root ? window : $scrollableEl;
