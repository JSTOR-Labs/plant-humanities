...
<param ve-image
	title="Image from 1720, showing the benefits that could be made from species circulation on temporary cleared fields (_chenas_). The image on the left shows a demarcated area of forest to be cultivated; the image on the middle shows the same field after the original vegetation was cleared; and the image on the right shows the _chena_ as a monoculture of cinnamon. The text at the bottom is a calculation of the expected returns: "
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4MTIyMDA0MzksMjA5OTU3Mzc4NSwxNT
